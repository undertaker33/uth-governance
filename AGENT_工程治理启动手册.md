...
项目或仓库会话开始时，先进入 `uth-governance` 做轻量场景判断。
...
2. 用户显式指定某个 `uth-*` 子 Skill 时，直接进入该子 Skill。
3. 没有工程动作信号，且没有任何 UTH 场景命中时，正常回答，不触发子 Skill。
4. 只有一个 UTH 场景命中时，输出一行简短判定并直接进入子 Skill。
5. 多场景命中时，选择第一个执行场景，后续场景留到收口时交接。
6. 场景仍不明确时，停下来问一个澄清问题。
...
Hook 是流程门禁，不是新研发流程。  
完整规则放在 `docs/_governance/hook-gates.md`；本仓库分发版可参考 `HOOKS_工程治理门禁手册.md`。
...
新项目启动时，Agent 执行 S0 项目接入场景。

步骤：

1. 读取 `AGENTS.md`，若不存在则准备创建。
...
1. 扫描当前结构和已有文档。
2. 总结技术栈、模块划分、启动方式、测试方式、配置和日志。
3. 不修改业务代码。
4. 补齐常驻上下文层，必要时为现有模块建立 `docs/context/`。
5. 建立 `docs/current-state.md`。
6. 为当前阶段建立一个任务包。
7. 将已有待办拆到任务包内的 Todo。
8. 如果会多窗口并行或使用 subagent，补齐 `docs/_governance/subagent-workflow.md`，并启用任务包 `prompts/` 规则。
9. 后续新需求全部走场景判定，再进入对应流程。
