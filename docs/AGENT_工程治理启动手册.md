...
3. **状态瘦身**：`entrypoints.current_state` 只做当前状态索引，不做开发日志；历史状态归档到 `docs/state/snapshots/`。
...
21. **文件名语言一致**：`AGENTS.md` 和每级目录入口 `README.md` 是硬入口文件名，保持英文；其他生成治理 Markdown 的文件名跟随初始化时选择的 `document_language`，实际路径写入 `.uth-governance/project.json` 的 `entrypoints`。
...
```text
entrypoints.current_state                 # 从 .uth-governance/project.json 读取，例如 docs/当前状态.md
docs/<localized-project-overview>.md
docs/<localized-architecture>.md
docs/<localized-development>.md
docs/context/README.md
docs/context/01-<localized-module>.md     # 00、01、...、09、10 两位编号
docs/<localized-api-contract>.md          # 可选
docs/<localized-data-model>.md            # 可选
docs/<localized-domain-glossary>.md       # 可选
docs/<localized-ui-guidelines>.md         # 可选
docs/<localized-deployment>.md            # 可选
```
...
├─ context/
│  └─ README.md
├─ <localized-api-contract>.md      # 可选，例如 docs/API契约.md
├─ <localized-data-model>.md        # 可选，例如 docs/数据模型.md
├─ <localized-domain-glossary>.md   # 可选，例如 docs/领域术语.md
├─ <localized-ui-guidelines>.md     # 可选，例如 docs/界面规则.md
├─ <localized-deployment>.md        # 可选，例如 docs/部署.md
│
├─ _governance/
│  ├─ README.md
│  ├─ agent-rules.md
...
| `entrypoints.current_state` | 当前状态索引，只保留最新活跃事实 |
| `docs/<localized-project-overview>.md` | 项目概览、目标用户、模块、非目标范围 |
| `docs/<localized-architecture>.md` | 当前架构、模块职责、调用边界 |
| `docs/<localized-development>.md` | 环境、启动、测试、构建、本地问题 |
| `docs/context/` | 模块级当前事实摘要，帮助 Agent 按需装载上下文 |
| `docs/snapshots/` | 项目级快照，例如老项目接管 handoff |
| `docs/<localized-api-contract>.md` | 当前 API 契约，例如 `docs/API契约.md` |
| `docs/<localized-data-model>.md` | 当前数据模型，例如 `docs/数据模型.md` |
| `docs/<localized-domain-glossary>.md` | 当前领域术语，例如 `docs/领域术语.md` |
| `docs/<localized-ui-guidelines>.md` | 当前 UI / UX 规则，例如 `docs/界面规则.md` |
| `docs/<localized-deployment>.md` | 当前部署与回滚方式，例如 `docs/部署.md` |
...
- `entrypoints.current_state` 只保留当前状态。
- 任务包完成时可生成 snapshot。
- 正式版本发布前可生成 snapshot。
...
- `uth-debug`：收口时必须判断 Git 收口建议；只读诊断通常结论为不建议，独立轻量修复完成并验证后判断是否建议，正式任务包内修复按 Design 级人类验收口径判断。
- `uth-design`：收口时必须判断 Git 收口建议；只读评估通常结论为不建议，稳定 Design / ADR / 经确认的小补丁成果可建议 Git 收口。
- `uth-docs`：收口时必须判断 Git 收口建议；只读或 blocked 文档治理通常结论为不建议，稳定文档治理成果或用户要求提交时可建议 Git 收口。
...
- 归档文件只作历史证据，不覆盖 `entrypoints.current_state`、`docs/context/` 或常驻上下文中的当前事实。
...
- 老项目 full-takeover preflight 后必须路由到独立的 `uth-docs onboarding-followup` 并停止 onboarding preflight；只有 docs 场景返回 `full-project-docs-complete` 和独立场景证据后才允许最终接管收口。
- 不执行 Git 写入。
...
```text
docs/<localized-development>.md
docs/<localized-api-contract>.md
docs/<localized-data-model>.md
docs/<localized-domain-glossary>.md
docs/decisions/
```
...
entrypoints.current_state                  # 只更新当前活跃设计索引
```
...
轻量改动如果没有当前正式任务包，可只读取 `AGENTS.md`、`entrypoints.current_state` 和相关代码/局部文档。
...
```text
改 API -> docs/<localized-api-contract>.md
改数据模型 -> docs/<localized-data-model>.md
改 UI -> docs/<localized-ui-guidelines>.md
改部署 -> docs/<localized-deployment>.md
```
...
- LW 记录默认不更新 `entrypoints.current_state`，除非影响当前阶段、阻塞、验证基线或发布判断。
...
docs/<localized-api-contract>.md
docs/<localized-data-model>.md
docs/<localized-ui-guidelines>.md
docs/<localized-development>.md
```
...
module-split：大型项目无法在单窗口可靠建立全项目基线时，先询问用户是否允许拆分；用户允许后写入带编号的 `docs/context/00-<localized-module-split>.md` 和 context 索引后暂停等待用户确认。
module-governance：在用户确认拆分后，按 `00-...md` 拆分计划的顺序连续治理模块；模块之间不暂停确认。
onboarding-followup：老项目完整接管事务中，承接 `uth-onboarding` handoff 并完成全项目文档治理。
state-cleanup：清理 `entrypoints.current_state` 指向的当前状态文档旧事实和旧索引。
...
- 老项目 full-takeover 只有在独立 `uth-docs onboarding-followup` 返回 `full-project-docs-complete` 和 docs 场景收口证据后，才允许 `uth-onboarding` 声明接管完成。
- `scoped-docs-complete` 只表示指定范围文档治理完成，不等价于全项目完成。

大型项目规则：

- 项目过大时先询问用户是否允许拆分；用户允许后进入 `module-split`，写 `docs/context/00-<localized-module-split>.md`、`docs/context/README.md` 和模块队列。
- 拆分后必须等用户确认，确认后再进入 `module-governance` 逐模块处理。
- `module-governance` 必须按 `00-...md` 拆分计划指定顺序连续执行；模块上下文文件使用 `01-...md`、...、`09-...md`、`10-...md` 两位编号；模块之间不暂停确认，除非出现 blocker、拆分需要修订或上下文过长。
...
- 拆分结果经用户确认后，再按模块队列连续创建 `01-...md`、`02-...md` 等模块上下文文件。
...
- `entrypoints.current_state` 是当前状态索引，不是日志。
...
- `uth-docs` 不执行 Git 写入；收口时必须写出 Git 收口判断；如果建议 commit / PR / tag / release，先询问用户是否进入 `uth-git`。
...
`entrypoints.current_state` 是当前状态索引，不是项目日志。
...
```text
docs/context/README.md
docs/context/00-<localized-module-split>.md   # 仅由 uth-docs module-split 在用户允许拆分后创建
docs/context/01-<localized-module>.md
docs/context/02-<localized-module>.md
```
...
7. 建立或更新 `entrypoints.current_state` 初始索引。
8. 创建 `.uth-governance/project.json`。
9. 不修改业务代码。
10. 不把旧 Design、旧 Todo、旧 Feedback、旧 Run Log 或 Prompt 当成当前事实。
11. full-takeover 必须路由到独立的 `uth-docs onboarding-followup`，由 `uth-docs` 基于代码事实执行 `full-project-baseline`、旧文档分类、context 重建、current-state 清理和归档处理；`uth-onboarding` 不得内联执行这些动作。
12. `uth-docs` 返回 `full-project-docs-complete` 和 docs 场景收口证据后，再回到 `uth-onboarding` 做最终接管收口。

老项目接管阶段的 `entrypoints.current_state` 只能是初始索引。没有读到足够代码事实时，写“待 `uth-docs` 根据代码事实深化确认”，不要写成已确认事实。
...
- 大型项目必须先询问是否允许 `module-split`；用户允许后写 `docs/context/00-<localized-module-split>.md`、context 报告和模块队列；用户确认拆分结果后按模块队列连续治理，不在模块间再次暂停确认。
