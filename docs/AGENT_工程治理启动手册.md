...
切到正式开发后，不能只声明 `formal-dev` 就开始实现。`formal-dev` / `todo-implementation` 必须先有 active task package、已接受的 Design 和当前 Todo；缺 Design 先回 `uth-design`，缺 Todo 先进入 `todo-breakdown`。
...
Web 页面、Web app screen、browser UI、frontend page layout 的设计工作由 `uth-design` 触发 `ui-ux-pro-max`；Android UI/UX 设计不因 UI/UX 字样自动触发 `ui-ux-pro-max`。
...
治理 Markdown 落文档前必须先完成 `uth-sp-brainstorming` 预检，并确认没有仍需向用户提问的问题；存在问题时先问用户，不得先落库。
...
- `uth-design` 切换到 `uth-dev` 前必须停下并向用户确认；Design 被接受不等于自动授权实现。
...
- subagent 模式必须记录 worker/evaluator 身份：谁造的问题谁解决，谁提出的问题谁复查；A 发现 B 的问题时，总控回派 B 修复，B 修完后回到 A 复查。
