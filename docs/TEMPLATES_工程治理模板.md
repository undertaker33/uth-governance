...
- `uth-onboarding` 必须写入 `.uth-governance/project.json`、复制项目本地 `tools/uth-hooks/`，并建立初始 `docs/current-state.md`；首次写治理 Markdown 或输出收口报告前必须询问并保存 `document_language`；老项目还必须先创建备份压缩包和接管快照，并自动交给 `uth-docs`。
