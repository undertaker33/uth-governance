...
- `AGENTS.md` 和所有目录入口 `README.md` 是硬入口文件名，保持英文。
- 其他生成的治理 Markdown 文件名必须跟随初始化时选择的 `document_language`。
- `.uth-governance/project.json` 的 `entrypoints` 必须记录实际路径；其他场景不得硬编码假定 `docs/current-state.md`。
- 模块上下文文件必须使用两位数字前缀：`00-...md`、`01-...md`、...、`09-...md`、`10-...md`。
- `docs/context/00-<localized-module-split>.md` 不是 onboarding 初始化项，只能由 `uth-docs module-split` 在用户允许大型项目拆分后创建。
...
1. `docs/README.md`
2. 本文件 `AGENTS.md`
3. `.uth-governance/project.json` 中的 `entrypoints.current_state`
...
- 项目概览：`docs/<localized-project-overview>.md`
- 当前状态：`.uth-governance/project.json` 的 `entrypoints.current_state`
- 架构与模块边界：`docs/<localized-architecture>.md`
- 本地启动、测试、构建：`docs/<localized-development>.md`
...
3. 如果基线不清、问题跨模块或涉及历史追溯，先进入文档定位或读取 `entrypoints.current_state` 指向的活跃任务包。
...
当前事实索引写入 `entrypoints.current_state` 和 `docs/context/`。
````
...
- `<localized-current-state>.md`：当前项目状态索引。
- `<localized-project-overview>.md`：项目概览、技术栈、模块、非目标范围。
- `<localized-architecture>.md`：当前架构、模块边界、调用关系。
- `<localized-development>.md`：本地环境、启动、测试、构建。
- `context/`：模块级当前事实摘要，帮助 Agent 按需装载上下文。
- `<localized-api-contract>.md`：API 契约，可选，例如 `API契约.md`。
- `<localized-data-model>.md`：数据模型，可选，例如 `数据模型.md`。
- `<localized-domain-glossary>.md`：领域术语，可选，例如 `领域术语.md`。
- `<localized-ui-guidelines>.md`：UI / UX 规则，可选，例如 `界面规则.md`。
- `<localized-deployment>.md`：部署说明，可选，例如 `部署.md`。
...
## 3. docs/<localized-current-state>.md 模板
...
- 先由文档治理窗口理解全仓并询问用户是否允许拆分。
- 用户允许后写 `00-<localized-module-split>.md`、context README/报告和模块队列。
- 经用户确认拆分结果后再创建 `01-...md`、`02-...md` 等模块上下文。
...
- `uth-onboarding` 必须写入 `.uth-governance/project.json`、复制项目本地 `tools/uth-hooks/`，并建立初始 `entrypoints.current_state`；首次写治理 Markdown 或输出收口报告前必须询问并保存 `document_language`；老项目还必须先创建备份压缩包和接管快照，并自动交给 `uth-docs`。
- `uth-docs` 修改治理 Markdown 前，必须确认 `.uth-governance/project.json` 已有 `document_language`，或本次已询问用户并持久化。
- 所有 UTH 场景收口报告必须按 `document_language` 输出；选择中文时不得使用 `Read`、`Created/updated`、`Current-state` 等英文字段标签。
...
项目级当前主线、模块索引和仓库特有硬约束写入根 `AGENTS.md`；当前事实写入 `entrypoints.current_state` 和 `docs/context/`。
...
`entrypoints.current_state` 指向的文档是当前状态索引，不是项目日志。
...
归档内容不是当前事实源；当前事实仍以 `entrypoints.current_state`、`docs/context/` 和常驻上下文文档为准。
