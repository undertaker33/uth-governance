---
name: uth-governance
description: Use at the start of a repository or project conversation when Codex must decide whether the request belongs to a UTH engineering scene, when a request may span multiple scenes, or when the correct uth-* child skill is unclear. Also use when the user asks to route, classify, or apply engineering governance. Do not use when the user explicitly invokes skill-creator.
---
...
2. Explicit child UTH skill named by user: enter that child skill.
3. No project action signal and no UTH scene condition matches: answer normally; no child skill.
4. Exactly one UTH scene matches: state one short scene line, then enter that child skill.
5. Multiple scenes match: choose the first execution scene and defer later scenes to closeout.
6. Scene still unclear: stop and ask one concise clarification question.
