...
Execute an accepted UTH formal development plan by dispatching a fresh worker per task, with review after each task. The owning `uth-dev` scene remains the controller for prompts, write scopes, verification, Feedback, current-state, and Git handoff.
