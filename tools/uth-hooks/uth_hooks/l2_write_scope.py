...
def check_hard_forbidden(path: str, ctx: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if match_pattern(path, "docs/context/**") and active_scene != "uth-docs":
        return result("BLOCK", "context-write-blocked", "docs/context/** is writable only in uth-docs.", path)

    if match_pattern(path, "docs/archive/**") and active_scene != "uth-docs":
        return result("BLOCK", "archive-write-blocked", "docs/archive/** is writable only in uth-docs.", path)
    ...
    if match_pattern(path, "docs/changelogs/**") and active_scene != "uth-git":
        return result("BLOCK", "changelog-write-blocked", "docs/changelogs/** writes require uth-git.", path)
